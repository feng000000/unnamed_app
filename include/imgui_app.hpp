...
// - Documentation        https://dearimgui.com/docs (same as your
// local docs/ folder).
// - Introduction, links and more at the top of imgui.cpp

// Important note to the reader who wish to integrate
// imgui_impl_vulkan.cpp/.h in their own engine/app.
// - Common ImGui_ImplVulkan_XXX functions and structures are used
// to interface with imgui_impl_vulkan.cpp/.h.
//   You will use those if you want to use this rendering backend
//   in your engine/app.
// - Helper ImGui_ImplVulkanH_XXX functions and structures are
// only used by this example (main.cpp) and by
//   the backend itself (imgui_impl_vulkan.cpp), but should
//   PROBABLY NOT be used by your own engine/app code.
// Read comments in imgui_impl_vulkan.h.

#ifndef __IMGUI_APP_HPP__
#define __IMGUI_APP_HPP__
...
SetupVulkan(ImVector<const char*> instance_extensions)
{
    ...
    // If you wish to load e.g. additional textures you may need
    // to alter pools sizes and maxSets.
    {
...
// All the ImGui_ImplVulkanH_XXX structures/functions are optional
// helpers used by the demo. Your real engine/app may not use
// them.
static void
SetupVulkanWindow(
...
FramePresent(ImGui_ImplVulkanH_Window* wd)
{
    ...
    wd->SemaphoreIndex = (wd->SemaphoreIndex + 1) %
                         wd->SemaphoreCount;  // Now we can use the next set of
                                              // semaphores
}
}  // namespace
...
public:
    ...
    {
        handle = true;
        printf("[Context] enter context %s\n", ctx_name);
...
protected:
    ...
    exit() noexcept
    {
        ...
        printf("[Context] exit context %s", ctx_name);
    }
...
private:
    ...
    execute_func_list(const char* identify, TaskList& task_list) noexcept
    {
        for (auto& func : task_list)
        {
            ...
            {
                func();
                printf("[Context] execute %s func success\n", identify);
            }
            catch (const std::exception e)
            {
                printf(
                    "[Context] execute %s func failed: %s\n", identify, e.what()
                );
            }
            catch (...)
            {
                printf(
                    "[Context] enter %s func throw unknown "
                    "exception\n",
                    identify
                );
            }
...
public:
    ...
    ImGuiApp(
        context::Context&& context,
        std::function<void(void)> main_loop_event,
        const char* app_name = "My Application",
        int window_width = 1280,
        int window_height = 800
    ) noexcept
        : context(std::move(context)), render(main_loop_event),
          app_name(app_name), window_width(window_width),
    ...
    // 移动构造
    ImGuiApp(ImGuiApp&& other) noexcept
        : context(std::move(other.context)), render(other.render),
          app_name(other.app_name), window_width(other.window_width),
    ...
    // 禁用移动赋值
    ImGuiApp&
    operator=(ImGuiApp&& other) = delete;
    ...
    // 禁用拷贝赋值
    ImGuiApp&
    operator=(const ImGuiApp&) = delete;

    int
    run()
    {
        // Setup SDL
        // [If using SDL_MAIN_USE_CALLBACKS: all code below until the
        // main loop starts would likely be your SDL_AppInit()
        // function]
        ...
        SDL_Window* window = SDL_CreateWindow(
            this->app_name,
            (int)(this->window_width * main_scale),
            (int)(this->window_height * main_scale),
            window_flags
        );
        if (window == nullptr)
        {
        ...
        if (SDL_Vulkan_CreateSurface(
                window, g_Instance, g_Allocator, &surface
            ) == 0)
        {
        ...
        style.FontScaleDpi = main_scale;  // Set initial font scale. (using
                                          // io.ConfigDpiScaleFonts=true makes
                                          // this unnecessary. We leave both
                                          // here for documentation purpose)
        io.ConfigDpiScaleFonts =
            true;  // [Experimental] Automatically overwrite
                   // style.FontScaleDpi in Begin() when Monitor DPI
                   // changes. This will scale fonts but _NOT_ scale
                   // sizes/padding for now.
        io.ConfigDpiScaleViewports =
            true;  // [Experimental] Scale Dear ImGui and Platform
                   // Windows when Monitor DPI changes.
        ...
        // Pass in your value of VkApplicationInfo::apiVersion,
        ...
        //   This selection is based on (style.FontSizeBase *
        //   style.FontScaleMain * style.FontScaleDpi) reaching a
        //   small threshold.
        // - You can load multiple fonts and use
        // ImGui::PushFont()/PopFont() to select them.
        // - If a file cannot be loaded, AddFont functions will return
        // a nullptr. Please handle those errors in your code (e.g.
        // use an assertion, display an error and quit).
        ...
        while (!done)
        {
            ...
            // io.WantCaptureKeyboard flags to tell if dear imgui
            // wants to use your inputs.
            // - When io.WantCaptureMouse is true, do not dispatch
            // mouse input data to your main application, or
            ...
            // and hide them from your application based on those two
            // flags. [If using SDL_MAIN_USE_CALLBACKS: call
            // ImGui_ImplSDL3_ProcessEvent() from your SDL_AppEvent()
            // function]
            SDL_Event event;
            ...
            // [If using SDL_MAIN_USE_CALLBACKS: all code below would
            // likely be your SDL_AppIterate() function]
            ...
            const bool main_is_minimized =
                (main_draw_data->DisplaySize.x <= 0.0f ||
        ...
        // Cleanup
        // [If using SDL_MAIN_USE_CALLBACKS: all code below would
        // likely be your SDL_AppQuit() function]
