...
static void SetupVulkan(ImVector<const char*> instance_extensions)
{
    ...
    // Create Vulkan Instance
    {
        ...
        err = vkCreateInstance(
            &create_info, g_Allocator, &g_Instance
        );
        check_vk_result(err);
#ifdef IMGUI_IMPL_VULKAN_USE_VOLK
    ...
    g_QueueFamily =
        ImGui_ImplVulkanH_SelectQueueFamilyIndex(g_PhysicalDevice
        );
    ...
    // to alter pools sizes and maxSets.
    {
        ...
        pool_info.poolSizeCount =
            (uint32_t)IM_COUNTOF(pool_sizes);
...
)
{
    ...
    {
        fprintf(
            stderr, "Error no WSI support on physical device 0\n"
        );
        exit(-1);
    }
...
static void CleanupVulkan()
{
    vkDestroyDescriptorPool(
        g_Device, g_DescriptorPool, g_Allocator
    );
    ...
    f_vkDestroyDebugReportCallbackEXT(
        g_Instance, g_DebugReport, g_Allocator
    );
#endif  // APP_USE_VULKAN_DEBUG_REPORT
...
int main(int, char**)
{
    ...
    style.FontScaleDpi =
        main_scale;  // Set initial font scale. (using
                     ...
                     // unnecessary. We leave both here for
    ...
    while (!done)
    {
        ...
        wd->ClearValue.color.float32[0] =
            clear_color.x * clear_color.w;
        wd->ClearValue.color.float32[1] =
            clear_color.y * clear_color.w;
        wd->ClearValue.color.float32[2] =
